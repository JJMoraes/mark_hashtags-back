...
public class UserService {
    ...
    public RequestTokenResponseDTO login(){
        ...
        return new RequestTokenResponseDTO(requestToken.getToken(), requestToken.getTokenSecret(), requestToken.getAuthorizationURL());
    }

    public String sign(AccessTokenDTO accessToken){
        twitter4j.User twitterUser = twitterAuthentication.requestUser(
                accessToken.getOauth_token(), accessToken.getOauth_tokenSecret(), accessToken.getOauth_verifier()
        );
        Optional<User> result = getByEmail(twitterUser.getEmail());
        User user = (result.isPresent())?result.get():save(new User(null, twitterUser.getEmail(), twitterUser.getScreenName(), null));
